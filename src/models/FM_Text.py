...
class Text_FM(nn.Module):
    ...
    def forward(self, x):
        ...
        # text to dense
        user_text_feature = self.text_embedding(
            user_text_vector
        )  # (batch_size, embed_dim)
        user_text_feature = user_text_feature.view(
            -1, 1, user_text_feature.size(1)
        )  # (batch_size, 1, embed_dim)
        item_text_feature = self.text_embedding(
            book_text_vector
        )  # (batch_size, embed_dim)
        item_text_feature = item_text_feature.view(
            -1, 1, item_text_feature.size(1)
        )  # (batch_size, 1, embed_dim)

        # second-order interaction / dense
        dense_feature = torch.cat(
            [user_book_embedding, user_text_feature, item_text_feature], dim=1
        )  # (batch_size, num_fields + 2, embed_dim)
...
class Text_DeepFM(nn.Module):
    ...
    def forward(self, x):
        user_book_vector, user_text_vector, book_text_vector = (
            x[0],
            x[1],
            x[2],
        )  # (batch_size, num_fields), (batch_size, word_dim), (batch_size, word_dim)
